...
namespace Sigtrap.VrTunnellingPro.Editors
{
    public abstract class VrTunnellingProEditorBase : Editor
    {
        ...
        protected SerializedProperty _pFxCoverX;
        protected SerializedProperty _pFxCoverY;
        protected SerializedProperty _pFxOffsetX;
        protected SerializedProperty _pFxOffsetY;
        protected SerializedProperty _pFxOriginX;
        protected SerializedProperty _pFxOriginY;
        ...
        #region Labels
        static readonly GUIContent _gcEffectCoverage_x = new GUIContent("X", "Effect Coverage in x dimension.");
        static readonly GUIContent _gcEffectCoverage_y = new GUIContent("Y", "Effect Coverage in y dimension.");
        static readonly GUIContent _gcEffectOrigin_x = new GUIContent("X", "Effect Origin in x dimension.");
        static readonly GUIContent _gcEffectOrigin_y = new GUIContent("Y", "Effect Origin in y dimension.");
        static readonly GUIContent _gcEffectOffset_x = new GUIContent("X", "Effect offset in x dimension.");
        static readonly GUIContent _gcEffectOffset_y = new GUIContent("Y", "Effect offset in y dimension.");
        ...
        static bool _showEffectCoverageSettings = true;
        static bool _showEffectOriginSettings = true;
        static bool _showEffectOffsetSettings = true;
        ...
        protected void OnEnable()
        {
            ...
            _pFxCoverX = serializedObject.FindProperty("effectCoverage_x");
            _pFxCoverY = serializedObject.FindProperty("effectCoverage_y");
            _pFxOffsetX = serializedObject.FindProperty("effectCoverageOffset_x");
            _pFxOffsetY = serializedObject.FindProperty("effectCoverageOffset_y");
            _pFxOriginX = serializedObject.FindProperty("effectOriginX");
            _pFxOriginY = serializedObject.FindProperty("effectOriginY");
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (_showEffectSettings)
            {
                ...
                _showEffectCoverageSettings = EditorGUILayout.Foldout(_showEffectCoverageSettings, "Effect Coverage Settings");
                if (_showEffectCoverageSettings)
                {
                    ++EditorGUI.indentLevel;
                    EditorGUILayout.BeginHorizontal();
                    EditorGUIUtility.labelWidth = 45.0f; // change the width of the property field
                    EditorGUILayout.PropertyField(_pFxCoverX, _gcEffectCoverage_x);
                    EditorGUILayout.PropertyField(_pFxCoverY, _gcEffectCoverage_y);
                    EditorGUILayout.EndHorizontal();
                    --EditorGUI.indentLevel;
                }
                _showEffectOriginSettings = EditorGUILayout.Foldout(_showEffectOriginSettings, "Effect Origin Settings");
                if (_showEffectOriginSettings)
                {
                    ++EditorGUI.indentLevel;
                    EditorGUILayout.BeginHorizontal();
                    EditorGUILayout.PropertyField(_pFxOriginX, _gcEffectOrigin_x);
                    EditorGUILayout.PropertyField(_pFxOriginY, _gcEffectOrigin_y);
                    EditorGUILayout.EndHorizontal();
                    --EditorGUI.indentLevel;
                }
                _showEffectOffsetSettings = EditorGUILayout.Foldout(_showEffectOffsetSettings, "Effect Offset Settings");
                if (_showEffectOffsetSettings)
                {
                    ++EditorGUI.indentLevel;
                    EditorGUILayout.BeginHorizontal();
                    EditorGUILayout.PropertyField(_pFxOffsetX, _gcEffectOffset_x);
                    EditorGUILayout.PropertyField(_pFxOffsetY, _gcEffectOffset_y);
                    EditorGUILayout.EndHorizontal();
                    --EditorGUI.indentLevel;
                }

                --EditorGUI.indentLevel;
                EditorGUIUtility.labelWidth = 0; // set labelWidth back to default
