...
namespace Sigtrap.VrTunnellingPro {
	...
	public class TunnellingMobile : TunnellingBase {
		...
		void LateUpdate () {
            ...
            (Vector2 motion,_) = CalculateMotion(Time.deltaTime);
            _irisMat.SetFloat(_propFxInner_x, motion.x);
            _irisMat.SetFloat(_propFxOuter_x, motion.x - effectFeather);
            _irisMat.SetFloat(_propFxInner_y, motion.y);
            _irisMat.SetFloat(_propFxOuter_y, motion.y - effectFeather);
