...
namespace Sigtrap.VrTunnellingPro
{
    ...
    public abstract class TunnellingBase : MonoBehaviour
    {
        ...
        protected const string PROP_OUTER_X = "_FxOuterX";
        protected const string PROP_INNER_X = "_FxInnerX";
        protected const string PROP_OUTER_Y = "_FxOuterY";
        protected const string PROP_INNER_Y = "_FxInnerY";
        protected const string PROP_ORIGIN_X = "_FxOriginX";
        protected const string PROP_ORIGIN_Y = "_FxOriginY";
        protected const string PROP_OFFSET_X = "_FxOffsetX";
        protected const string PROP_OFFSET_Y = "_FxOffsetY";
        protected const string PROP_Transparency = "_Transparency";
        ...
        const float COVERAGE_MIN_x = 0.40f;
        const float COVERAGE_MIN_y = 0.40f;
#else
        //const float COVERAGE_MIN = 0.65f;
        const float COVERAGE_MIN_x = 0.45f;
        const float COVERAGE_MIN_y = 0.45f;
#endif
        #endregion
        ...
        public float effectCoverage_x = COVERAGE_MIN_x;
        [Range(0f, 1f)]
        [Tooltip("Maximum screen coverage.")]
        public float effectCoverage_y = COVERAGE_MIN_y;
        [Range(-1f, 1f)]
        [Tooltip("Screen coverage offset in x direction.")]
        public float effectCoverageOffset_x = 0.0f;
        [Range(-1f, 1f)]
        [Tooltip("Screen coverage offset in y direction.")]
        public float effectCoverageOffset_y = 0.0f;
        [Range(-0.5f, 0.5f)]
        [Tooltip("Screen coverage origin in x dimension.")]
        public float effectOriginX = 0.0f;
        [Range(-0.5f, 0.5f)]
        [Tooltip("Screen coverage origin in y dimension.")]
        public float effectOriginY = 0.0f;
        //public Vector2 virtualMotionSpeed = new Vector2(0.0f,0.0f);
        //public bool showScalarRestrictor = false;
        //public float scalarRestrictorSize = 0.0f;
        ...
        // transparency level for restrictors based on distance from user to obstacles in physical world
        ...
        protected int _propFxInner_x, _propFxOuter_x, _propFxInner_y, _propFxOuter_y, _propFxOrigin_x, _propFxOrigin_y, _propFxOffset_x, _propFxOffset_y, _transparency;
...
#pragma warning restore 0414
        ...
        protected virtual void Awake()
        {
            ...
            _propFxOuter_x = Shader.PropertyToID(PROP_OUTER_X);
            _propFxInner_x = Shader.PropertyToID(PROP_INNER_X);
            _propFxOuter_y = Shader.PropertyToID(PROP_OUTER_Y);
            _propFxInner_y = Shader.PropertyToID(PROP_INNER_Y);
            _propFxOrigin_x = Shader.PropertyToID(PROP_ORIGIN_X);
            _propFxOrigin_y = Shader.PropertyToID(PROP_ORIGIN_Y);
            _propFxOffset_x = Shader.PropertyToID(PROP_OFFSET_X);
            _propFxOffset_y = Shader.PropertyToID(PROP_OFFSET_Y);
            _transparency = Shader.PropertyToID(PROP_Transparency);
        ...
        protected void ApplyPresetBase(TunnellingPresetBase p)
        {
            ...
            if (p.overrideEffectCoverage) { effectCoverage_x = p.effectCoverage_x; effectCoverage_y = p.effectCoverage_y; }
        ...
        protected Vector2 RemapRadius(float x)
        {
            Vector2 xy;
            xy.x = Mathf.Lerp(COVERAGE_MIN_x, 1, x);
            xy.y = Mathf.Lerp(COVERAGE_MIN_y, 1, x);
            return xy;
        }

        protected (Vector2, float) CalculateMotion(float dT)
        {
            ...
            if (useAngularVelocity)
            {
                ...
                angularMotion = _avSmoothed * angularVelocityStrength;
                fx += angularMotion;
            }
            ...
            fx = RemapRadius(fx).x * RemapRadius(effectCoverage_x).x;
            float fy = RemapRadius(fx).y * RemapRadius(effectCoverage_y).y;
            ...
            if (_debugForceOn)
            {
                motion.x = Mathf.Clamp01(RemapRadius(_debugForceValue).x * RemapRadius(effectCoverage_x).x);
                motion.y = Mathf.Clamp01(RemapRadius(_debugForceValue).y * RemapRadius(effectCoverage_y).y);
